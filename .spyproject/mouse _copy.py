...
def calibrate_2(color, color_threshold, color_index):  # color_index [0,1,2]=[r,g,b]

        name = "Calibrate" + color
        cv2.namedWindow(name)
        cv2.createTrackbar("Threshold Value", name, color_threshold, 255, nothing)
        cv2.createTrackbar("Tune", name, 0, 255, nothing)
        while True :
            _, img = cap.read()
            img = cv2.flip(img, 1)
            ...
            channel_img = cv2.split(img)
            img_diff = channel_img[color_index] - img_gray
            thres = cv2.getTrackbarPos("Threshold Value", name)
            _, img_bin = cv2.threshold(img_diff, thres, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
            img_bin = img_bin * channel_img[color_index]
            tune = cv2.getTrackbarPos("Tune", name)
        # print(tune,thres)
            img_bin[img_bin < tune] = 0
            img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)

            imgx = morph_img(img,img_bin)
    ...
    if perform:
         ...
         if action == 'move':
            if yp[0]>110 and yp[0]<590 and yp[1]>120 and yp[1]<390:
                pyautogui.moveTo(cursor[0],cursor[1],1)
...
color_threshold = [68,70, 72]
#blue_th = calibrate_2('Blue', color_threshold[2], 2)
#green_th = calibrate_2('Green', color_threshold[1], 1)
red_th = calibrate_2('Red', color_threshold[0], 0)
yellow_range = calibrateColor('Yellow', yellow_range)
#red_range = calibrateColor('Red', red_range)
blue_range = calibrateColor('Blue', blue_range)
